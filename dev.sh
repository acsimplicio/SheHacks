...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=shehacks.example.com
...
workon shehacks  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development shehacks npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop shehacks
    docker rm shehacks
    docker run --name shehacks -d --env-file /home/ubuntu/shehacks.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/shehacks:/dkdata \
        shehacks start_web.sh
}
...
echo_green "Welcome to shehacks's dev env"
